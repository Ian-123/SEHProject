...
def save_data(properties, filename="properties.json"):
    def convert(obj):
        ...
        # IMPORTANT: never return obj itself
        # fall back to a string (or raise TypeError)
        return str(obj)
...
def find_circular(obj):
    ...
    def walk(x, path):
        ...
        if isinstance(x, (dict, list)):
            if xid in seen:
                return seen[xid], path
            seen[xid] = path

        if isinstance(x, dict):
            for k, v in x.items():
                res = walk(v, path + [f"[{repr(k)}]"])
                if res:
                    return res
...
if edit_i is not None and 0 <= edit_i < len(existing_tx):
    ...
    with st.form("edit_tx_form"):
        ...
        with ec2:
            ...
            loan_term_years_e = st.number_input("Loan term (years)", value=float(tx.get("loan_term_years") or 0.0), step=0.5, min_value=0.0)
...
with st.form("tx_form"):
    ...
    with c2:
        ...
        st.number_input("Loan term (years)", step=0.5, min_value=0.0, key="tx_loan_term_years")
...
if upload and st.button("Import now"):
    ...
    res = find_circular(properties)
    if res:
        first, again = res
        st.error("❌ Circular reference detected in properties")
        st.write("First seen at:", "".join(first))
        st.write("Seen again at:", "".join(again))
        st.stop()
